...
FHIR-Proxy only exerts control as a Pre- or Post- processor if it has been configured to do so. Otherwise, API calls made to the FHIR-Proxy endpoint go straight through to the FHIR service, and responses go directly back to the remote client app (e.g., Postman).
