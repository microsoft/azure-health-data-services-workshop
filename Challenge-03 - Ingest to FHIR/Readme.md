...
2. What happens as a result? Go ahead and unzip the `bad_bundles.zip` archive. Open and inspect the contained JSON bundle in a text editor (e.g., VS Code). You can compare this JSON bundle from the `bad_bundles.zip` archive with the JSON bundles in the `good_bundles.zip` archive.
