...
Visit the FHIR-Bulk Loader (OSS) repository [here](https://github.com/microsoft/fhir-loader) and read the documentation for instructions on how to upload bulk FHIR data into Azure API for FHIR using FHIR-Bulk Loader (the process simply involves uploading files to a blob storage container in Azure).
