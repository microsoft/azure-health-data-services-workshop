...
Along with Elements, each FHIR Resource is defined with a set of search parameters. When a client app makes API calls, adding search parameters to the request allows fine-grained Element data retrieval from Resources stored on a FHIR server. There are standard search parameters that apply to all Resources (e.g., `_id`, `_lastUpdated`), and there are Resource-specific search parameters (e.g., `gender` is a Resource-specific search parameter defined for `Patient`). The links below go into detail about the standard search parameters in FHIR and the `Patient` Resource-specific search parameters. 
