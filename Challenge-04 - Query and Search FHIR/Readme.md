...
## FHIR Search methods
When you do a search in FHIR, you need to set the scope of information that you wish to query. You can narrow the scope of a FHIR search to be within a range defined by
...
The simplest way to execute a search in FHIR is to use a `GET` request. For example, if you want to pull all patients in a FHIR server database, you could send a request with the search criteria narrowed by the `Patient` Resource type: 
