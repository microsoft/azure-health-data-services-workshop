...
## Background
The FHIR specification defines a REST API with syntax for querying Resources in a FHIR server's data store. When a client app seeks access to FHIR Resources (e.g., on behalf of a patient or care provider), the app sends the FHIR server a query for information within the specified search parameters, and if that information (e.g., PHI) is within the app's scope of permissions, the server carries out the search and returns the results. The FHIR standard allows for much flexibility in setting search criteria, and in this challenge we will get practice with the various methods of searching for FHIR Resources in Azure API for FHIR.   
