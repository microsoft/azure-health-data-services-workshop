...
But it would be more efficient to retrieve all of this information in a single query. This capability is available in the `_include` and `_revinclude` parameters. The example below illustrates how `_include=` specifies the referring Resource (`AllergyIntolerance`) followed by the target Resource (`patient` is short for `subject:Patient`). 
