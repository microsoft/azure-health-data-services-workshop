...
## Background
The FHIR specification defines a REST API with syntax for querying Resources in a FHIR server's data store. When a client app requests access to Personal Health Information (PHI) on a FHIR server (e.g., on behalf of a patient or care provider), the app sends a query for data within certain parameters, and if the app is permitted access, the server returns the search results. The FHIR standard allows for much flexibility in setting search criteria, and in this challenge we will get practice with the various methods of searching for FHIR Resources in Azure API for FHIR. 
