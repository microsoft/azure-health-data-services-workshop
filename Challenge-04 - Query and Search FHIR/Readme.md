...
Within every Resource, FHIR defines a set of Elements for storing details that uniquely identify each Resource *instance* on a FHIR server. Elements such as `id` and `meta` are used for all Resource types in FHIR, while other Elements are attached to specific Resource types (e.g., the `gender` Element is only found in `Patient`, `Person`, `Practitioner`, and `RelatedPerson` Resources). Furthermore, the FHIR model is designed to allow users to add Elements to Resources through extensions.
...
You can also call the FHIR search API with `POST`, which is useful if the query string is too long or if the query must be hidden to conceal Personal Health Information (PHI). To search using `POST`, the search parameters are delivered to the FHIR server in JSON in the body of the request.

When a search request is successful, if it's a single-instance search (e.g., `GET {{fhirurl}}/Patient/123`), you'll receive a JSON-formatted Resource instance in return. If it's a request for more than one Resource instance or a query formed with search parameters, you’ll receive a JSON-formatted FHIR `Bundle` response containing the search results. If the search request fails, you’ll find the error details in the `"OperationOutcome"` part of the response.

## Common Search Parameters 

The following search parameters are available for all FHIR Resources: ```_content```, ```_id```, ```_lastUpdated```, ```_profile```, ```_query```, ```_security```, ```_source```, and ```_tag```.  Moreover, the ```_text```, ```_filter```, and [search result parameters](https://www.hl7.org/fhir/search.html#Summary) work with all Resources.

The search parameter ```_id``` is used to specify the [Logical ID](https://www.hl7.org/fhir/resource.html#id) of a Resource instance on a FHIR server (the `Patient` Resource type is used as an example here):
