...
Along with Elements, each FHIR Resource is defined with a set of search parameters. When a client app makes API calls, adding search parameters to the request enables fine-grained Element data retrieval from Resources stored on a FHIR server. There are standard search parameters that apply to all Resources (e.g., `_id`, `_lastUpdated`), and there are Resource-specific search parameters (e.g., `gender` is a Resource-specific search parameter defined for `Patient`). The links below go into detail about the standard search parameters and the `Patient` Resource-specific search parameters. 
...
You can also search using `POST`, which is useful if the query string is too long for a single line. To search using `POST`, the search parameters can be submitted in the body of the request. This makes it easier to form a longer, more complex series of query parameters.
...
The search parameter ```_id``` refers to the logical id of the Resource (the `id` Element value) and can be used when the search context specifies a Resource type:
