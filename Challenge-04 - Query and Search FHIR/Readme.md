...
The simplest way to execute a search in FHIR is to use a `GET` request. For example, if you want to pull all Patient Resources in the FHIR server database, you would query for the Resource type: 

```azurecli
GET {{FHIR_URL}}/Patient
```

If you want to retrieve the Resource instance for a specific patient, you could narrow your search within the Resource type by including the `_id` search parameter: 
...
The search parameter ```_id``` refers to the logical id of a Resource instance and can be used when the search context specifies a Resource type:
