...
## Background
The FHIR specification defines a REST API with syntax for querying Resources in a FHIR server's data store. When a client app seeks access to FHIR Resources (e.g., on behalf of a patient or care provider), the app queries the FHIR server, and if the query is within the app's permitted scope of access, the server carries out the search and returns the results in a [FHIR Resource Bundle](http://hl7.org/fhir/bundle.html). The FHIR standard offers a variety of options for fine tuning search criteria, and in this challenge we will get practice with different methods of finding information in the FHIR service database. 
...
The FHIR standard offers a comprehensive set of search parameters for filtering query results. 
