...
But it would be more efficient to retrieve all of this information in a single query. This capability is made available in the `_include` and `_revinclude` parameters. The example below illustrates how `_include=` expands the main search (`AllergyIntolerance?_code=`) by specifying referenced Resource instances also to be returned (`patient` is short for `subject:Patient`). 
