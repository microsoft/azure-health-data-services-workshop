...
1. Using the FHIR Search collection in Postman, conduct several chained searches and then try a reverse-chained search using the ```_has``` parameter. For more examples of chained and reverse-chained searches, refer to the **[FHIR search examples](https://docs.microsoft.com/en-us/azure/healthcare-apis/fhir/search-samples)** page.
